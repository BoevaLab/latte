...
def test_subspace_fit() -> None:
    ...
    d_12 = evaluation.subspace_fit(U_1.numpy(), U_2.numpy())
    d_13 = evaluation.subspace_fit(U_1.numpy(), U_3.numpy())
    d_23 = evaluation.subspace_fit(U_2.numpy(), U_3.numpy())
...
def test_mixing_matrix_fit() -> None:
    ...
    assert evaluation.mixing_matrix_fit(A_1, A_2)["Value"]["Difference between matrices"] > 0
    assert (
        pytest.approx(0, abs=50 * 1e-4)
        == evaluation.mixing_matrix_fit(A_1, A_3)["Value"]["Difference between matrices"]
        > 0
    )
